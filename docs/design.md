...
- **Библиотека — app-shell:** страница = `h-[calc(100svh-4rem)] flex-col overflow-hidden` (высота минус хедер); скроллится только зона списка (`flex-1 min-h-0 overflow-y-auto`). Шапка экрана — всегда компактная: строка «Библиотека + Показано N из M + вид (Плитка/Список)» (`text-lg`), под ней тулбар фильтров (`h-8` контролы, `border-b`, без sticky — он и так не скроллится); пагинация — закреплённая нижняя панель (`border-t`). Скелетон (`LibrarySkeleton`) повторяет всю рамку (шапка/тулбар/скролл-зона/панель), сетка — из общего `BOOKS_GRID_CLASS`, чтобы контент не прыгал.
- **Тулбар фильтров библиотеки:** одна компактная строка — поиск (с кнопкой очистки), пилюли статусов (активная — заливка primary), «Теги» (Popover с чипами и счётчиком), рейтинг и сортировка — Select, «Сбросить». На мобильных — Sheet с тем же содержимым.
